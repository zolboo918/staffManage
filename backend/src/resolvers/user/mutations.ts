...
const userMutations = {
  createUser: async (_, args: { user: User }) => {
    const newUser = await prisma.user.create({ data: args.user });
    return newUser;
  },
  updateUser: async (_, args: { id: string; user: User }) => {
  ...
  register: async (
    _,
    args: { email: string; password: string; isAdmin: boolean }
  ) => {
  ...
  sendResetPasswordTokenEmail: async (_, args: { email: string }) => {
    const user = await prisma.user.findFirst({
      where: { email: args.email },
    });
    if (!user) {
  ...
  checkPasswordToken: async (_, args: { email: string; token: string }) => {
    const user = await prisma.user.findFirst({
  ...
  changePassword: async (_, args: { email: string; newPassword: string }) => {
